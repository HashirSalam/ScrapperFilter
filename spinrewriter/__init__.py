...
class Api(object):
    ...
    _tmp_list = ['ok', 'error']
    STATUS = namedtuple('STATUS', ['ok', 'error'])(*map(str.upper, _tmp_list))
    ...
    def _send_request(self, params):
        ...
        :rtype: dictionary
        """
        con = urllib2.urlopen(self.URL, urllib.parse.urlencode(params).encode("utf-8"))
        response = con.read()
    ...
    def _transform_plain_text(self, action, text, protected_terms, confidence_level,
                              nested_spintax = False, spintax_format = 'pipe_curly'):
